...
## Notizie locali — Udine (06/09/2026)

Dopo Trieste, l'utente ha chiesto la seconda provincia con 5 fonti indicate esplicitamente: UdineToday.it, UDINE.news (udineoggi.news), Telefriuli.it (categoria "/cronaca/"), PrimaUdine.it (categoria "/category/in-evidenza") e di nuovo RaiNews TGR FVG (pagina tag Udine, URL fornito direttamente dall'utente con il tag già estratto: `Udine|Tag-104004c3-3848-4a0e-b70b-470105b903e9`).

**Verifica delle 5 fonti prima di scrivere codice**, stesso principio di sempre:

1. **UDINE.news** — ✅ feed RSS 2.0 standard verificato via WebFetch su `/feed/`, stesso formato data RFC 2822 già gestito da `ingestNotizieFonteRss()`.
2. **Telefriuli.it** — ✅ l'utente ha indicato solo la categoria "/cronaca/": verificato che esiste un feed dedicato (`/cronaca/feed/`), con contenuto diverso dal feed generale del sito (non solo un alias) — usato quello invece del feed sitewide.
3. **PrimaUdine.it** — ⚠️ l'utente ha indicato la categoria "/category/in-evidenza", ma il sito non pubblica un feed RSS per singola categoria: provati sia `/category/in-evidenza/feed/` (404) sia `?feed=rss2` (torna la pagina HTML, non un feed) — esiste solo un feed per l'intero sito (`/feed/`, verificato via WebFetch). **Usato il feed generale come approssimazione più ampia** della sola "in evidenza" — l'aggregatore mostrerà quindi tutte le notizie del sito, non solo quelle segnalate come in evidenza. Se si preferisce restare filtrati alla sola categoria, serve l'outerHTML reale della pagina categoria (WebFetch non è bloccato qui, ma riassume sempre in markdown e non basta per scrivere selettori cheerio di produzione).
4. **RaiNews TGR FVG** — ✅ stessa API di ricerca già integrata per Trieste il giorno prima (vedi sezione sopra), cambia solo il valore di `tagRainews` — l'utente ha fornito direttamente l'URL con il tag Udine già estratto, nessuna nuova verifica di formato necessaria (stesso endpoint, stessa struttura risposta).
5. **UdineToday.it** — ⚠️ **non verificabile direttamente**: stessa rete Citynews di TriestePrima.it (stesso tema, perfino lo stesso identico percorso URL "/notizie/tutte/"), bloccata sia dal fetch/curl diretto di questo sandbox sia da WebFetch stesso (403 client error) — esattamente come TriestePrima.it prima che l'utente fornisse l'outerHTML reale il 05/09/2026. **Scelta presa**: riusare gli stessi selettori già verificati su TriestePrima.it (`article.c-story`, `a.o-link-text` con `aria-label`, `.c-story__byline span`) generalizzando lo scraper dedicato (rinominato `ingestNotizieFonteCitynews`, con un nuovo campo `baseUrl` per ricostruire correttamente i link relativi per ciascun dominio) invece di lasciare la fonte completamente fuori — una scommessa ragionevole data l'identità di piattaforma, non una verifica vera e propria. **Se il primo run reale desse 0 elementi per UdineToday.it**, servirà l'outerHTML reale della pagina da questo dominio specifico (stesso identico metodo già usato con successo per TriestePrima.it) per confermare o correggere i selettori — nessun rischio per le altre fonti nel frattempo, stesso fallback "array vuoto, non un errore" già in uso per tutto il modulo Notizie.

**Implementazione**: `FONTI_NOTIZIE_UDINE` in `scripts/ingest-light.mjs`, aggiunta a `PROVINCE_NOTIZIE` accanto a Trieste; `PROVINCE_NOTIZIE_ATTIVE` in `lib/notizieProvincia.ts` estesa a `["trieste", "udine"]` — nessuna altra modifica necessaria alla UI (`NotizieProvinciaPage.tsx` era già generica sulle province attive/in arrivo, solo il commento in cima aggiornato). Generalizzazione della funzione ex-`ingestNotizieFonteTriestePrima()` (ora `ingestNotizieFonteCitynews()`, riusata da TriestePrima.it e UdineToday.it) verificata con uno script cheerio a sé stante contro un frammento HTML ricostruito sulla stessa identica struttura già confermata reale per TriestePrima.it, con due `baseUrl` diversi — link relativi ricostruiti correttamente per ciascun dominio, link già assoluti preservati senza doppio prefisso, articolo senza titolo/link scartato senza errori.
