...
| **Calcio** — pagina dedicata `/calcio` | gare.lnd.it (LND Comitato FVG) | App Inertia.js: la pagina incorpora l'intero stato (partite + classifica) in un tag `<script data-page="app">`, estratto con `cheerio` — niente scraping di HTML visibile. Solo Eccellenza Girone A per ora (`COMPETIZIONI_CALCIO` in `ingest-light.mjs`, estendibile ad altri campionati). Raggiungibile dal menu ad amburger in alto a sinistra |
