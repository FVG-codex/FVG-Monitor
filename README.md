...
| **Baseball** — pagina dedicata `/baseball` | fibs.it (FIBS) | Dati nell'HTML servito dal server, ma il sito **blocca il fetch senza uno User-Agent che assomigli a un browser reale** (`HEADERS_FIBS` in `ingest-light.mjs`) — senza quello, la richiesta viene rifiutata come bot. Solo Serie A Silver per ora, squadra FVG di riferimento: GEREON Engineering NBP Ronchi (Ronchi dei Legionari). Estendibile via `COMPETIZIONI_BASEBALL` |
