...
| **Baseball** — pagina dedicata `/baseball` | fibs.it (FIBS) | **Unico modulo sul workflow "pesante"** (`ingest-heavy.yml`, ogni 30 min): fibs.it blocca un fetch semplice con HTTP 403 anche imitando gli header di un browser — serve un browser reale (Playwright/Chromium), che invece passa. Script separato: `scripts/ingest-heavy.mjs`. Solo Serie A Silver per ora, squadra FVG di riferimento: GEREON Engineering NBP Ronchi (Ronchi dei Legionari). Estendibile via `COMPETIZIONI_BASEBALL` in quello script |
