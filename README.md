...
| **Radar meteo** | API monitoraggio PC FVG, gruppo `radar` | Solo il radar di Fossalon (id 1) è attivo — Lussari e Mosaico risultano spenti (`status: "X"`). Prodotto `SRTLBM_1` (PNG, intensità pioggia in mm) tra i tanti disponibili sullo stesso radar (velocità Doppler, classificazione idrometeore, ecc., non ancora usati). L'immagine radar è **trasparente fuori dalle zone di pioggia** (nessuna base geografica) — sovrapposta a una vera mappa (Leaflet + tile OpenStreetMap) usando l'`extent` geografico fornito dall'API stessa (`RadarMeteoMap.tsx`, caricato dinamicamente lato client — Leaflet richiede il DOM del browser) |
| **Viabilità** — pagina dedicata `/viabilita` + pannello homepage | InfoViaggiando (eventi, feed WFS non dichiarato pubblico — stessa cautela di ANSA) + OSMER (webcam A4/A23/A28/SR354) | La pagina dedicata combina il pannello eventi (`ViabilitaPanel`, stesso dato del pannello homepage) con le webcam autostradali filtrate dallo stesso snapshot `webcam:osmer` usato da `/webcam` |
